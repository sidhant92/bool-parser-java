    ...
    public Object evaluate(final List<EvaluatedNode> items) {
        if (items
            ...
            return ValueUtils.caseDouble(items
                                                 .stream().mapToDouble(a -> Double.parseDouble(a.getValue().toString())).sum());
        }
        if (items
                .stream().anyMatch(a -> a.getDataType().equals(DataType.LONG))) {
            return ValueUtils.caseDouble(items
                                                 .stream().mapToLong(a -> Long.parseLong(a.getValue().toString())).sum());
        }
        return ValueUtils.caseDouble(items
                                             .stream().mapToInt(a -> Integer.parseInt(a.getValue().toString())).sum());
    }
