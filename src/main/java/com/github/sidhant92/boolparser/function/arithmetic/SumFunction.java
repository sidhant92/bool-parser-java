    ...
    public Object evaluate(final List<Pair<Object, DataType>> items) {
        if (items
            ...
            return ValueUtils.caseDouble(items
                                                 .stream().mapToDouble(a -> Double.parseDouble(a.getKey().toString())).sum());
        }
        if (items
                .stream().anyMatch(a -> a.getValue().equals(DataType.LONG))) {
            return ValueUtils.caseDouble(items
                                                 .stream().mapToLong(a -> Long.parseLong(a.getKey().toString())).sum());
        }
        return ValueUtils.caseDouble(items
                                             .stream().mapToInt(a -> Integer.parseInt(a.getKey().toString())).sum());
    }
